...
+(NSImageView *)readFileAtPathAsImageView:(NSString *)path
{
    return [self readFileAtPathAsImageView:path error:nil];
}


+(NSImageView *)readFileAtPathAsImageView:(NSString *)path error:(NSError **)error
{
    NSImage *image = [self readFileAtPathAsImage:path error:error];
...
+(BOOL)writeFileAtPath:(NSString *)path content:(NSObject *)content error:(NSError **)error
{
    ...
    else if([content isKindOfClass:[NSImage class]])
    {
        ...
        NSData *pngData = [newRep representationUsingType:NSPNGFileType properties:[NSDictionary new]];
        [pngData writeToFile:path atomically:YES];
    }
    else if([content isKindOfClass:[NSImage class]])
    {
        return [self writeFileAtPath:absolutePath content:((NSImageView *)content).image error:error];
    }
